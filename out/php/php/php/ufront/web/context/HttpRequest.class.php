...
class php_ufront_web_context_HttpRequest extends ufront_web_context_HttpRequest {
	...
	public function get_postString() {
		...
		if($this->postString === null) {
			...
			if($this->postString !== null) {
				$this->postString = urldecode($this->postString);
			} else {
				$this->postString = "";
			}
		}
	...
	public function parseMultipart($onPart = null, $onData = null, $onEndPart = null) {
		...
		if($this->_parsed) {
			throw new HException(new tink_core_TypedError(null, "parseMultipart() can only been called once", _hx_anonymous(array("fileName" => "HttpRequest.hx", "lineNumber" => 59, "className" => "php.ufront.web.context.HttpRequest", "methodName" => "parseMultipart"))));
		}
		$this->_parsed = true;
		$post = $this->get_post();
		if(isset($_FILES)) {
					...
					if(isset($_POST)) {
						$postNames = new _hx_array(array_keys($_POST));
						{
							...
							while($_g < $postNames->length) {
								...
								$val = $_POST[$name];
								if(is_array($val)) {
									if(null == php_Lib::hashOfAssociativeArray($val)) throw new HException('null iterable');
									$__hx__it = php_Lib::hashOfAssociativeArray($val)->iterator();
									while($__hx__it->hasNext()) {
										$innerVal = $__hx__it->next();
										if(is_string($innerVal)) {
											ufront_core__MultiValueMap_MultiValueMap_Impl_::add($this->post, $name, urldecode($innerVal));
										}
									}
								} else {
									if(is_string($val)) {
										ufront_core__MultiValueMap_MultiValueMap_Impl_::add($this->post, $name, php_ufront_web_context_HttpRequest_7($this, $_g, $name, $postNames, $val));
									}
								}
								unset($val,$name);
							}
		...
		if($this->clientHeaders === null) {
			...
			while($__hx__it->hasNext()) {
				...
				if(_hx_substr($k, 0, 5) === "HTTP_") {
					$headerName = thx_core_Strings::capitalizeWords(php_ufront_web_context_HttpRequest_8($this, $h, $k), null);
	...
	static function getHashFromString($s) {
		$qm = new haxe_ds_StringMap();
		{
			...
			while($_g < $_g1->length) {
				...
				ufront_core__MultiValueMap_MultiValueMap_Impl_::add($qm, php_ufront_web_context_HttpRequest_9($_g, $_g1, $part, $qm, $s), php_ufront_web_context_HttpRequest_10($_g, $_g1, $part, $qm, $s));
				unset($part);
			}
		}
		return $qm;
...
function php_ufront_web_context_HttpRequest_3(&$_g, &$allPartFutures, &$bsize, &$currentPos, &$err, &$errors, &$file, &$fileResource, &$info, &$name, &$onData, &$onEndPart, &$onPart, &$part, &$partFinishedTrigger, &$parts, &$post, &$tmp, $surprise, $andThen) {
	{
		call_user_func_array($surprise, array(array(new _hx_lambda(array(&$_g, &$allPartFutures, &$andThen, &$bsize, &$currentPos, &$err, &$errors, &$file, &$fileResource, &$info, &$name, &$onData, &$onEndPart, &$onPart, &$part, &$partFinishedTrigger, &$parts, &$post, &$surprise, &$tmp), "php_ufront_web_context_HttpRequest_11"), 'execute')));
	}
}
...
function php_ufront_web_context_HttpRequest_6(&$allPartFutures, &$errors, &$onData, &$onEndPart, &$onPart, &$parts, &$post, $_5) {
	{
		if($errors->length === 0) {
			...
			return tink_core_Outcome::Failure(tink_core_TypedError::withData(null, "Error parsing multipart request data", $errors, _hx_anonymous(array("fileName" => "HttpRequest.hx", "lineNumber" => 151, "className" => "php.ufront.web.context.HttpRequest", "methodName" => "parseMultipart"))));
		}
	}
}
function php_ufront_web_context_HttpRequest_7(&$__hx__this, &$_g, &$name, &$postNames, &$val) {
	{
		$s = $val;
		return urldecode($s);
	}
}
function php_ufront_web_context_HttpRequest_8(&$__hx__this, &$h, &$k) {
	{
...
function php_ufront_web_context_HttpRequest_10(&$_g, &$_g1, &$part, &$qm, &$s) {
	{
		$s2 = php_ufront_web_context_HttpRequest::$paramPattern->matched(2);
		return urldecode($s2);
	}
}
function php_ufront_web_context_HttpRequest_11(&$_g, &$allPartFutures, &$andThen, &$bsize, &$currentPos, &$err, &$errors, &$file, &$fileResource, &$info, &$name, &$onData, &$onEndPart, &$onPart, &$part, &$partFinishedTrigger, &$parts, &$post, &$surprise, &$tmp, $outcome) {
	{
...
function php_ufront_web_context_HttpRequest_12(&$_g, &$allPartFutures, &$bsize, &$buf, &$currentPos, &$err, &$errors, &$file, &$fileResource, &$info, &$name, &$onData, &$onEndPart, &$onPart, &$part, &$partFinishedTrigger, &$parts, &$post, &$processResult, &$readNextPart, &$readNextPart1, &$size, &$tmp) {
	{
		call_user_func($readNextPart1);
	}
}
function php_ufront_web_context_HttpRequest_13(&$_g, &$allPartFutures, &$bsize, &$currentPos, &$err, &$errors, &$file, &$fileResource, &$info, &$name, &$onData, &$onEndPart, &$onPart, &$part, &$partFinishedTrigger, &$parts, &$post, &$processResult, &$readNextPart, &$readNextPart1, &$tmp) {
	{
		unlink($tmp);
		{
