...
class ufront_handler_ErrorPageHandler implements ufront_app_UFErrorHandler{
	...
	public function handleError($httpError, $ctx) {
		$callStack = "";
		{
			$msg = "Handling error: " . Std::string($httpError) . _hx_string_or_null($callStack);
			$ctx->messages->push(_hx_anonymous(array("msg" => $msg, "pos" => _hx_anonymous(array("fileName" => "ErrorPageHandler.hx", "lineNumber" => 54, "className" => "ufront.handler.ErrorPageHandler", "methodName" => "handleError")), "type" => ufront_log_MessageType::$Error)));
		}
...
function ufront_handler_ErrorPageHandler_0(&$__hx__this, $error, $showStack) {
	{
		...
		$callStackItems = ufront_handler_ErrorPageHandler::errorStackItems(haxe_CallStack::callStack());
		$exceptionStack = null;
		...
		if($showStack && $callStackItems->length > 0) {
			$callStack = "<div class=\"error-call-stack\"><h3>Call Stack:</h3>\x0A\x09\x09\x09\x09\x09<pre><code>" . _hx_string_or_null($callStackItems->join("\x0A")) . "</pre></code>\x0A\x09\x09\x09\x09</div>";
		} else {
			$callStack = "";
		}
		$content = "\x0A\x09\x09\x09<summary class=\"error-summary\"><h1 class=\"error-message\">" . Std::string($error) . "</h1></summary>\x0A\x09\x09\x09<details class=\"error-details\"> " . _hx_string_or_null($inner) . " " . _hx_string_or_null($pos) . " " . _hx_string_or_null($exceptionStack) . " " . _hx_string_or_null($callStack) . " </details>\x0A\x09\x09";
		return $content;
	}
}
function ufront_handler_ErrorPageHandler_1(&$__hx__this, $title, $content) {
	{
		return "<!DOCTYPE html>\x0A<html>\x0A<head>\x0A\x09<title>" . _hx_string_or_null($title) . "</title>\x0A\x09<link href=\"//netdna.bootstrapcdn.com/bootstrap/3.0.0/css/bootstrap.min.css\" rel=\"stylesheet\" />\x0A\x09<style>\x0A\x09\x09p[frown] {\x0A\x09\x09\x09text-align: center;\x0A\x09\x09}\x0A\x09\x09p[frown] span { \x0A\x09\x09\x09transform: rotate(90deg); \x0A\x09\x09\x09display: inline-block; \x0A\x09\x09\x09color: #bbb; \x0A\x09\x09\x09font-size: 3em;\x0A\x09\x09}\x0A\x09</style>\x0A</head>\x0A<body style=\"padding-top: 30px;\">\x0A\x09<div class=\"container\">\x0A\x09\x09<div class=\"jumbotron\">\x0A\x09\x09\x09<p frown><span>:(</span></p>\x0A\x09\x09\x09" . _hx_string_or_null($content) . "\x0A\x09\x09</div>\x0A\x09</div>\x0A</body>\x0A</html>";
	}
}
