...
class ufront_remoting_RemotingHandler implements ufront_app_UFRequestHandler{
	...
	public function remotingError($e, $httpContext) {
		$httpContext->messages->push(_hx_anonymous(array("msg" => $e, "pos" => _hx_anonymous(array("fileName" => "RemotingHandler.hx", "lineNumber" => 199, "className" => "ufront.remoting.RemotingHandler", "methodName" => "remotingError")), "type" => ufront_log_MessageType::$MError)));
...
function ufront_remoting_RemotingHandler_1(&$apiCallFinished, &$args, &$doneTrigger, &$httpContext, &$params, &$path, &$r, &$remotingCall, &$remotingResponse, &$u, $data) {
	{
		$s1 = new haxe_Serializer();
		$s1->serialize($data);
		return "hxr" . _hx_string_or_null($s1->toString());
...
function ufront_remoting_RemotingHandler_3(&$args, &$doneTrigger, &$e, &$httpContext, &$path, &$r, &$remotingResponse, $response) {
	{
